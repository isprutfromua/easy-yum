...
const cleanMarkers = () => {
  state.markers.forEach((m) => {
    toRaw<google.maps.Marker>(m).setMap(null);
  });
  state.markers = [];
};

const markerIcon = computed(() => {
  const mealType = Object.keys(iconsMap).find(
    (el) => el.indexOf(state.query) > -1
  );

  return mealType ? iconsMap[mealType as keyof typeof iconsMap] : null;
});

const getMarkerUrl = () =>
  markerIcon.value ? require("./assets/" + markerIcon.value + ".svg") : null;

//markers end

type Restaurant = google.maps.places.PlaceResult;
...
type TState = {
  ...
  openedWindow: google.maps.InfoWindow | null;
  query: string;
};

const filteredRestaurants = computed(() => {
  if (!state.priceLevel) return state.restaurants;

  return state.restaurants.flatMap((res) =>
    res.price_level === state.priceLevel ? res : []
  );
});
...
const setPrice = (price: number) => {
  state.priceLevel = state.priceLevel === price ? null : price;
...
const handleInputSubmit = async (query: string) => {
  state.restaurants = await searchPlaces(query);
  state.query = query;
};
  ...
  getUserGeolocation()
    .then(async (coords) => {
      ...
      await placeMarker(center.value, {
        icon: require("./assets/markers/person.png"),
      });
    })
    .catch((err) => {
      error.value = err;
...
watch(filteredRestaurants, (updatedRestaurants) => {
  cleanMarkers();
  drawDirection(null);
  ...
  updatedRestaurants.forEach((m, i) => {
    const pr = new Promise<void>((res) => {
      setTimeout(async () => {
        ...
        const marker = await placeMarker(markerCoordinates, {
  ...
  Promise.all([...promises]).then(() => {
        ...
        infoWindow.setContent(
          `<div class="app-info-window">
            ...
            <div class="app-info-window__details">
              ...
              <span class="app-info-window__address">${
                filteredRestaurants.value[id].formatted_address
              }</span>
              <span class="app-info-window__rating">${
                filteredRestaurants.value[id].rating
              }</span>
            </div>
          </div>`
        );
    ...
    if (firstMarkerPosition) {
      map.value?.panTo(firstMarkerPosition);
...
const updateMode = (mode: google.maps.TravelMode) => {
  currentMode.value = mode;
};

const draw = () => {
  if (!state.openedRestaurant?.geometry?.location || !currentMode.value) {
    throw new Error("Cant draw direction");
  }
...
<template>
  <AppHeader @submit:value="handleInputSubmit" />
  <main class="container">
    <aside class="filters">
      <AppFilterButton
        v-for="index in 4"
        :class="{ 'is-active': state.priceLevel === index }"
        :label="'$'.repeat(index)"
        :key="`filter-${index}`"
    ...
    <section class="info">
      ...
      <div style="overflow-y: auto; height: 100%">
        <AppRestaurant
          v-if="state.openedRestaurant"
          :restaurant="state.openedRestaurant"
        >
          <AppDistanceInfo
            :destination="state.openedRestaurant.geometry"
            @mode:update="updateMode"
          />
          <div style="display: flex; column-gap: 8px; justify-content: center">
            <AppBuildRouteButton @click="selectRestaurant(null)">
              Back
            </AppBuildRouteButton>
            <AppBuildRouteButton @click="draw" />
          </div>
        </AppRestaurant>
        <template v-if="filteredRestaurants.length">
          <div v-show="!state.openedRestaurant">
            <AppRestaurant
